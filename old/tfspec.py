...
#     n = math.floor(np.true_divide(np.true_divide(nt,10), sampling))
    ...
    dpss_tapers = np.float32(dpss_tapers)
    if dn is None :
    ...
    #                     extendedArray = extendArrayWithCurrentData(mX, 0, nch, False)
                ...
                inputArray = np.einsum('ij,ki->ijk',tmp,dpss_tapers,dtype=np.float32) # N x nch x k
                Xk = np.complex64(np.fft.fft(inputArray,axis=0,n=int(nf)))
                Xk = Xk[lowerBound:upperBound,]
                XkSquare = np.float32((Xk * np.conj(Xk)).real)
